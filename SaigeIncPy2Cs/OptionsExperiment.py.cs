        ...
        public void MyTrade() {
            ...
            foreach (var i in slice.OptionChains) {
                ...
                if (!this.Portfolio.Invested) {
                    ...
                    var expiry = Math.Min(expiries, key: x => abs((x.date() - this.Time.date()).days - 40));
                    var strikes = (from i in puts
                        select i.Strike).ToList();
                    // determine at-the-money strike
                    var strike = Math.Min(strikes, key: x => abs(x - underlying_price));
                    // compute probability
                    ...
                    if (optionStyle == "short_iron_condor") {
                        otm_call_strike = min(strikes, key: x => abs(x - underlying_price + p2sd * underlying_price));
                        var atm_call_strike = min(strikes, key: x => abs(x - underlying_price + p1sd * underlying_price));
                        var atm_put_strike = min(strikes, key: x => abs(x - underlying_price + m1sd * underlying_price));
                        otm_put_strike = min(strikes, key: x => abs(x - underlying_price + m2sd * underlying_price));
