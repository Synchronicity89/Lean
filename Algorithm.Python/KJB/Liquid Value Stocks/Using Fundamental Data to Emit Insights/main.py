...
class LiquidValueStocks(QCAlgorithm):
    ...
    def Initialize(self):
        # self.SetStartDate(2007, 5, 15)
        # self.SetEndDate(2012, 5, 15)
...
class LiquidValueUniverseSelectionModel(FundamentalUniverseSelectionModelKJB):
    ...
    def SelectFine(self, algorithm, fine):
        ...
        #universe = sortedByYields[:1] + sortedByYields[-1:]
...
class LongShortEYAlphaModel(AlphaModel):
    ...
    def Update(self, algorithm, data):
        ...
        if self.lastMonth == algorithm.Time.month:
            #return empty insights if the month has not changed.
            return insights
        #month must have changed so save current month
        self.lastMonth = algorithm.Time.month
