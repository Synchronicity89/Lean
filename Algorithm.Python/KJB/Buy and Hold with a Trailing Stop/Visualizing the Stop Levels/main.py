class BootCampTask(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        self.Debug("Levels Asset Price" +str(self.Securities["SPY"].Price))
        if (self.Time - self.stopMarketOrderFillTime).days < 15:
            return

        if not self.Portfolio.Invested:
            self.MarketOrder("SPY", 500)
            self.stopMarketTicket = self.StopMarketOrder("SPY", -500, self.highestSPYPrice * 0.9)
            ...
            self.Debug("Levels Stop Price" + str(self.Securities["SPY"].Price * 0.9))
