...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class BasicTemplateOptionBullPutSpreadAlgorithm : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void OnData(Slice slice)
        {
            if (!Portfolio.Invested)
            {
                ...
                if (slice.OptionChains.TryGetValue(_optionSymbol, out chain))
                {
                    ...
                    if (atmStraddle != null)
                    {
                        Sell(OptionStrategies.BullPutSpread(
                            _optionSymbol, 
                            atmStraddle.Strike + 5.00M,
                            atmStraddle.Strike - 5.00M,
                            atmStraddle.Expiry), 
                            2);
                    }
